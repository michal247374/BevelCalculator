...
begin
  ...
  {Obliczeniowe naprê¿enia gn¹ce}
  ObliczNaprezGnace;

  {Napre¿enia krytyczne przy zginaniu}
  NaprezKrytycznePrzyZginaniu;

  {Wspo³czynnik trwa³oœci}
  WspolTrwalosci;

  {Wspo³czynnik uwzglêdniaj¹cy wp³yw dwustronego przek³adania obci¹¿enia na z¹b}
  WspolYA;

  {Wspo³czynnik uwzglêdniaj¹cy chropowatoœæ powierzchni przejsciowej zêba i jej obróbke ciepln¹}
  WspolYR;

  {Wspó³czynnik uwzglêdaj¹cy wymiary kó³}
  WspolYX;

  {Naprê¿enia dopuszczalne dla obliczeñ sprawdzaj¹cych}
  NaprezGnaceSprawdz;

  {Warunek wytrzyma³oœci gn¹cej zêbów}
  WarunekWytrzGnacej;

end;

procedure TfrmMain.btnDalej5Click(Sender: TObject); //Przejœcie do Etapu5, obliczenia dla Etap6
begin
  tshEtap6.TabVisible:= true;
  pgcMain.TabIndex:=5;  //Odblokowuje i prze³¹cz na 5 zak³adke

  {Naprê¿enia stykowe przy przeci¹¿eniach}
  NaprezStykPrzyPrzeciaz;

  {Naprê¿enia gn¹ce przy przeci¹¿eniach}
  NaprezGnacePrzyPrzeciaz;

  {Moment rzeczywisty na wyjœciu}
  MomentRzecz;
...
begin
  ...
  case intObrobkaIndeks1 of
    -1 :  raise Exception.Create('Nie uda³o sie wczytac indeksu materia³u');
     ...
     else raise Exception.Create('le zdefiniowana obróbka cieplna materia³u');
  end;
...
procedure TfrmMain.NaprezKrytycznePrzyZginaniu;  //Okreœla napre¿enia krytyczne przy zginaniu na podstawie obóbki cieplnej materia³u
var
  intObrobkaIndeks1, intObrobkaIndeks2: Integer;
  iniDane: TIniFile;
begin
...
var
  ...
  mf: Integer;
begin
  //Obliczenie wspo³czynnika KFE uwglednaj¹cego zmiane obci¹¿enia napêdu w czasie
  O1:=sedObciazenie1.Value/100;
  O2:=sedObciazenie2.Value/100;
  O3:=sedObciazenie3.Value/100;
  t1:=sedCzas1.Value/100;
...
procedure TfrmMain.WspolYX; //Oblicza wspó³czynnik uwzglêdaj¹cy wymiery kó³
begin
